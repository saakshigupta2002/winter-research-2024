...
def extract_single_sheet(file_path, sheet_name, status_column_name):
    ...
    # Find the correct column for Nameplate Capacity
    nameplate_capacity_col = next((col for col in df.columns if 'Nameplate Capacity' in col), None)
    
    new_df = pd.DataFrame({
        'Region': df.iloc[first_data_row:, 0],
        'Site Name': df.iloc[first_data_row:, 2],
        'Technology Type': df.iloc[first_data_row:, 4],
        'Nameplate Capacity': df.iloc[first_data_row:, df.columns.get_loc(nameplate_capacity_col)] if nameplate_capacity_col else '',
    ...
    # Remove rows only if all three essential columns are missing
    new_df = new_df[~(new_df['Technology Type'].isna() & 
                      new_df['Nameplate Capacity'].isna() & 
                      new_df[status_column_name].isna())]
...
def process_sheet(df, region, sheet_type, status_column_name):
    ...
    for index, row in df.iterrows():
        ...
        # Include the row if at least one of the essential columns has a value
        if technology_type or nameplate_capacity or unit_status:
            new_row = {
